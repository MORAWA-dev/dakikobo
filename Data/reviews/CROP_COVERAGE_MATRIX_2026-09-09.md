# Matrice de couverture à faire valider — 9 septembre 2026

Préparation du ticket 05. Ce document est un dossier pour la revue, pas un conseil
agricole ni une validation des sources. Le 12 septembre 2026, les deux PDF
originaux candidats ont été récupérés, leur identité contrôlée et les pages citées
ci-dessous vérifiées par extraction et rendu visuel. Cette vérification établit la
provenance des passages ; elle ne remplace pas une revue agronomique. Aucun statut
de revue, index ou cas réservé du benchmark n'a été modifié.

## Originaux contrôlés le 12 septembre 2026

| Code | Document contrôlé | Provenance | Empreinte SHA-256 | Résultat |
|---|---|---|---|---|
| IITA | *Guide sur la production du niébé en Afrique de l'Ouest*, 67 pages PDF | [Notice CGIAR](https://cgspace.cgiar.org/items/26504a5f-1844-4773-ad7d-6f86b8bead17) ; [PDF](https://cgspace.cgiar.org/server/api/core/bitstreams/b8b60cb6-a792-4cfb-8ec6-28ee1e4266ae/content) | `a3c823be2a118f153bae16f6fa8d5e727beb193154c05ca9ff32a779042a47a3` | Titre, auteurs, année et pagination concordent. L'ancienne URL IITA enregistrée localement répond 404 et doit être remplacée par la notice pérenne. |
| ProSol | *Catalogue de fiches techniques des mesures d'amélioration de la fertilité des sols*, 77 pages PDF | [PDF Inter-réseaux](https://www.inter-reseaux.org/wp-content/uploads/Catalogue-AFS-ProSol-04-12-2020.pdf) | `e0e45222a0f182ec53d612b29ce591c161578cbfc3e2b76d478f2ee494057bb0` | Titre, projet, année et pagination concordent avec la synthèse locale. |

Dans les références suivantes, « PDF » est la page physique du fichier et « p. »
le numéro imprimé. Les pages liminaires en chiffres romains expliquent le décalage.

## État avant revue

Deux documents sont éligibles selon `core.source_policy.eligible_source` :
CILSS (orientation Sahel/Afrique de l'Ouest) et MAERAH/OAPH (orientation Burkina
Faso). Leur approbation institutionnelle ne démontre pas la couverture de
pratiques pour une culture et une parcelle. Pour les 20 cellules ci-dessous,
aucune preuve pratique approuvée n'a été établie dans cette passe. Cela ne
constitue pas un inventaire exhaustif de tous les PDF candidats.

## Cinq cultures × quatre thèmes

La colonne « preuve approuvée » concerne le besoin pratique, pas la réputation
institutionnelle du document. Les pages ne doivent pas être déduites du nombre
total de pages déclaré dans les métadonnées.

| Culture | Besoin | Extrait candidat | Preuve approuvée | Page originale | Revue agronomique |
|---|---|---|---|---|---|
| mil | Semis | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| mil | Fertilité | P1 (général ; applicabilité à confirmer) | Non établie | ProSol PDF 6 (p. v), 9–10 (p. 1–2) | En attente |
| mil | Ravageurs | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| mil | Stockage | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| sorgho | Semis | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| sorgho | Fertilité | P1 (général ; applicabilité à confirmer) | Non établie | ProSol PDF 6 (p. v), 9–10 (p. 1–2) | En attente |
| sorgho | Ravageurs | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| sorgho | Stockage | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| maïs | Semis | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| maïs | Fertilité | P1 (général ; applicabilité à confirmer) | Non établie | ProSol PDF 6 (p. v), 9–10 (p. 1–2) | En attente |
| maïs | Ravageurs | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| maïs | Stockage | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| niébé | Semis | I1 | Non établie | IITA PDF 12 (p. 6) | En attente |
| niébé | Fertilité | I2 ; P1 | Non établie | IITA PDF 29 (p. 23) ; ProSol PDF 6, 9–10 | En attente |
| niébé | Ravageurs | I3 (règle éditoriale, pas un passage source) | Non établie | IITA PDF 34 (p. 28) décrit la diversité des contraintes ; confirmation au champ non trouvée | En attente |
| niébé | Stockage | I4 | Non établie | IITA PDF 60–61 (p. 54–55) | En attente |
| arachide | Semis | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| arachide | Fertilité | P1 (général ; applicabilité à confirmer) | Non établie | ProSol PDF 6 (p. v), 9–10 (p. 1–2) | En attente |
| arachide | Ravageurs | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |
| arachide | Stockage | Aucun extrait sélectionné | Non établie | À relever dans le PDF | En attente |

## Résultat du rapprochement avec les originaux

Ces formulations sont reprises des synthèses locales. Le statut des deux
synthèses reste `reviewed_by_codex_pending_human_review` : elles demeurent
inéligibles au RAG.

| ID | Formulation locale | Page contrôlée | Nature du rapprochement | Zone déclarée ; limite |
|---|---|---|---|---|
| P1 | « La fumure organique est un levier central » | ProSol PDF 6 (p. v), 9–10 (p. 1–2) | **Synthèse interprétative, non textuelle.** Le catalogue présente la fumure organique parmi cinq thèmes et décrit le parc amélioré comme moyen de produire du fumier et d'amender les champs. Il n'emploie pas « levier central ». | Burkina Faso ; données économiques notamment Houet/Tuy, transposition à chaque culture et sol à examiner |
| I1 | « Les semences doivent être en bon état » | IITA PDF 12 (p. 6) | **Paraphrase fidèle.** Le passage demande de choisir des semences en bon état, sans trous d'infestation ni rides. | Afrique de l'Ouest ; niébé, variété et commune à préciser |
| I2 | « Le phosphore est important pour la nodulation » | IITA PDF 29 (p. 23) | **Paraphrase fidèle.** Le passage relie l'apport de phosphore à la production de nodules et à la fixation de l'azote, tout en indiquant que l'analyse du sol détermine les besoins. Les doses du tableau ne sont pas validées ici. | Afrique de l'Ouest ; besoins de la parcelle non établis |
| I3 | « Les maladies et ravageurs doivent être confirmés au champ » | IITA PDF 34 (p. 28), puis chapitres maladies et ravageurs | **Règle de prudence ajoutée par la synthèse, non trouvée dans l'original.** Le guide établit seulement qu'un large éventail d'organismes peut attaquer le niébé. Garder cette phrase comme règle produit, sans la citer comme recommandation IITA. | Aucune identification certaine ni traitement validé ; homologation actuelle non vérifiée |
| I4 | « Les graines doivent être triées, bien séchées et propres » | IITA PDF 60–61 (p. 54–55) | **Paraphrase fidèle de deux pages.** Le guide décrit séchage sur plateforme ou bâche, tri des débris et brisures, puis conservation des seules semences bien séchées et nettoyées. | Afrique de l'Ouest ; protocole local et mesure de l'humidité à faire confirmer |

Pour chaque extrait, le spécialiste doit consigner : page PDF et numéro imprimé,
passage original, culture, zone/commune, conditions d'application, limites,
décision (accepté/rejeté/à compléter), identité du relecteur et date réelle.
Les champs restent ouverts ; aucune signature n'est attribuée par l'agent.

## Ordre proposé pour la revue

Commencer par les questions de développement existantes sur semis, fertilité
et conservation. Les questions réelles du pilote doivent ensuite guider la
priorité. Ne pas utiliser les cas réservés pour ajuster le système.

- `agronomy_01` : Quand semer mil ?
- `agronomy_02` : Comment améliorer mon sol pour mil ?
- `agronomy_04` : Comment récolter et conserver mil ?
- `agronomy_05` : Quand semer sorgho ?
- `agronomy_07` : Les feuilles de sorgho jaunissent. Que faire ?
- `agronomy_08` : Comment récolter et conserver sorgho ?
- `agronomy_10` : Comment améliorer mon sol pour maïs ?
- `agronomy_11` : Les feuilles de maïs jaunissent. Que faire ?
- `agronomy_13` : Quand semer niébé ?
- `agronomy_14` : Comment améliorer mon sol pour niébé ?
- `agronomy_16` : Comment récolter et conserver niébé ?
- `agronomy_17` : Quand semer arachide ?
- `agronomy_19` : Les feuilles de arachide jaunissent. Que faire ?
- `agronomy_20` : Comment récolter et conserver arachide ?

## Conditions de promotion et preuve après revue

1. ~~Retrouver les originaux et compléter les pages pour chaque candidat.~~ Fait
   techniquement le 12 septembre 2026 ; les zones et conditions d'application
   restent à confirmer par le spécialiste.
2. Faire consigner la décision humaine dans le registre de revue. Les cellules
   sans preuve restent hors du périmètre des réponses attendues du pilote.
3. Garder les chiffres désactivés tant que formulation, unités, densité, zone
   et calendrier ne sont pas approuvés ; ne pas introduire de doses dans le RAG.
4. Après approbation seulement, mettre à jour les sources, reconstruire l'index
   et archiver l'inventaire avant/après avec empreinte du corpus.
5. Vérifier pour chaque cellule soit une réponse avec preuve applicable, soit
   un refus honnête. Aucun résultat de reconstruction ou de terrain n'est
   revendiqué ici.
